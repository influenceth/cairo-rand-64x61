...
describe('simplex3d', function () {
  this.timeout(600_000);
  let contract;

  before(async () => {
    const contractFactory = await starknet.getContractFactory('Simplex3DMock');
    contract = await contractFactory.deploy();
  });

  it.only('should return accurate results for noise', async () => {
    const argsList = [
      [ 0.0, 0.0, 0.0 ],
      [ 0.5, -1.23, 1.63 ],
      [ -1.94, -1.25, -1.63 ],
    ...
    for (const [ i, args ] of argsList.entries()) {
      ...
      const { res } = await contract.call('Simplex3D_noise_test', { v });
